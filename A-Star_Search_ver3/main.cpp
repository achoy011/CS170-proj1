...
// Refereences from cplusplus.com
// VERSION 3
// HEY HEY IT WORKS
// But then I realized that the code won't be affected by heuristic and will
// always be breadth-first
// I have to revert to version 2

// The reason why I changed everything was because test case 3 was causing it to 
// go on for infinity so I didn't know what was going on
// ughhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA
...
// Function prototypes
// MAIN FUNCTIONS
bool aStar(vector<Node>&, vector<Node>&, int &);
int heuristic(Node);
bool testState(const Node &, const Node &);
void expand(vector<Node>, vector<Node> &, int);
...
int main(int argc, char** argv) {
    ...
    vector<Node> q, checkQ;
    checkQ.push_back(initial);
    int pos;
    
    if(aStar(q, checkQ, pos)) {
...
// ================================================
bool aStar(vector<Node> &q, vector<Node> &checkQ, int &pos) {
    ...
    do 
    {
        ...
        pos = max(static_cast<int>(q.size()-1), 0);
        /*cout << "curr depth: " << q[pos].gn << endl;
        cout << "curr pos: " << pos << endl;*/
        // Test if the new front-most node is the goal state
        for(vector<Node>::iterator it = checkQ.begin(); it != checkQ.end(); it++) {
            if(testState(goal, (*it)) && (*it).hn == 0)  {
                q.insert(q.end(), checkQ.begin(), checkQ.end());
                pos += checkQ.end()-it;
                checkQ.clear();
                return true;
            }
...
// ===========================================================================
void expand(vector<Node> q, vector<Node> &checkQ, int pos) {
    // Array that holds the x/y transforms to find adjacent tile positions
    // to help with looping operators
    ...
    for(int i = 0; i < 4; i++) {
        ...
        if((adjPos.first >= 0 && adjPos.first < 3) && (adjPos.second >= 0 && adjPos.second < 3)) {
            ...
            for(vector<Node>::iterator it = q.begin(); it != q.end(); it++) {
                if(testState((*it), newNode)) {
                    cout << "NEW NODE ALREADY IN Q:" << endl;
                    for(int y = 0; y < 3; y++) {
                        for(int x = 0; x < 3; x++) {
                            cout << newNode.state[x][y] << " ";
                        }
                        cout << endl;
                    }
                    alreadyThere = true;
            ...
            if(!alreadyThere) {
                for(vector<Node>::iterator it = checkQ.begin(); it != checkQ.end(); it++) {
                    if(testState((*it), newNode)) {
                        cout << "NEW NODE ALREADY IN CHECKQ:" << endl;
                            for(int y = 0; y < 3; y++) {
                                for(int x = 0; x < 3; x++) {
                                    cout << newNode.state[x][y] << " ";
                                }
            ...
            if(!alreadyThere) checkQ.push_back(newNode);
