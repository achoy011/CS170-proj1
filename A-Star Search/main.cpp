...
// VERSION 1
// Issues: debug output is incorrect, results are incorrect with test case 1
...
// State node
struct Node {
    char state[3][3];   // Puzzle grid, 0 represents the blank space
    // gn = depth = path cost
...
bool testState(const Node &node1, const Node &node2) {
    ...
    for(int i = 0; i < 3; i++) {
        for(int j = 0; j < 3; j++) {
            if(node1.state[i][j] != node2.state[i][j]) return false;
        }
    }
    return true;
}
